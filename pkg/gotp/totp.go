...
func (totp *TOTP) ProvisionUri(name, issuer, image string) string {
	return BuildUri(totp.secret, name, issuer, totp.hasher.name, image, &totp.digits, &totp.Interval, nil)
}
